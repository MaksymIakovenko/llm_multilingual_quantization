...
def load_tokenizer_only(model_path, hf_token=HF_TOKEN):
    tokenizer = AutoTokenizer.from_pretrained(model_path, token=hf_token)
    return tokenizer


# def load_unemb_only(model_size, path="."):
...
#     if model_size == "tiny":
#         lm_head_state_dict = load_file(
#             f"{path}/TinyLlama-1.1B-intermediate-step-1431k-3T/model.safetensors"
#         )
...
class LlamaHelper(BaseHelper):
    def __init__(
        ...
        self.model = AutoModelForCausalLM.from_pretrained(
            ...
            load_in_8bit=bnb_quant == 8,
            load_in_4bit=bnb_quant == 4,
        )
        if device:
...
class GPTQLlamaHelper(BaseHelper):
    def __init__(
        self,
        dir="/dlabdata1/llama2_hf/Llama-2-7b-hf",
        hf_token=None,
        ...
        if device is None:
            ...
            self.device = device
        self.tokenizer = AutoTokenizer.from_pretrained(dir, use_auth_token=hf_token)
        self.q_model = AutoGPTQForCausalLM.from_quantized(
            dir, use_auth_token=hf_token, device_map=device_map
        )
        self.model = self.q_model.model
        self.use_embed_head = True
        W = list(self.model.model.embed_tokens.parameters())[0].detach()
        self.head_embed = torch.nn.Linear(W.size(1), W.size(0), bias=False)
        self.head_embed.to(W.dtype)
...
class AWQLlamaHelper(BaseHelper):
    def __init__(
        self,
        dir="/dlabdata1/llama2_hf/Llama-2-7b-hf",
        hf_token=None,
        ...
        if device is None:
            ...
            self.device = device
        self.tokenizer = AutoTokenizer.from_pretrained(dir, use_auth_token=hf_token)
        self.q_model = AutoAWQForCausalLM.from_quantized(
            dir, use_auth_token=hf_token, device_map=device_map, fuse_layers=False
        )
        self.model = self.q_model.model
        self.use_embed_head = True
        W = list(self.model.model.embed_tokens.parameters())[0].detach()
