...
def plot_ci_plus_heatmap(
    ...
    fig, (ax, ax2) = plt.subplots(
        nrows=2, sharex=True, gridspec_kw={"height_ratios": [1, 10]}, figsize=(10, 5)
    )
    if do_colorbar:
...
def plot_ci(
    ...
    if method == "gaussian":
        ...
        std = data.std(dim=0)
        data_ci = {
            "x": np.arange(data.shape[1]) + 1 + layer_idx_offset,
            "y": mean,
    ...
    if do_confidence:
        ax.fill_between(
            df["x"], df["y_lower"], df["y_upper"], color=color, alpha=confidence_alpha
        )
    process_axis(ax)
